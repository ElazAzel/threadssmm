...
export function AccountsPage() {
  ...
  const { workspace, accounts: workspaceAccounts, brands, addManualAccount, refresh } = useWorkspace()
  ...
  const connectThreads = async () => {
    ...
    try {
      ...
      const result = await fetch('/api/threads/connect', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ workspaceId: workspace.id }) })
      const payload = await result.json() as { url?: string; error?: string }
      if (!result.ok || !payload.url) throw new Error(payload.error || 'Не удалось открыть Meta OAuth')
    ...
    <AppShell>
      ...
      <div className="accounts-layout">
        ...
        {account ? <Card className="account-detail">
          ...
          <div className="split-actions"><Button variant="secondary" onClick={() => void refresh()}><RefreshCw size={17} /> Обновить</Button><Button variant="secondary" onClick={() => window.open(`https://www.threads.net/@${account.username}`, '_blank', 'noopener,noreferrer')}><ExternalLink size={17} /> Открыть Threads</Button></div>
...
export function StudioPage() {
  ...
  const { workspace, createQuickDraft, requestApproval, refresh } = useWorkspace()
  ...
  const generate = async () => {
    ...
    try {
      ...
      const result = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({ workspaceId: workspace.id, prompt, format: contentFormat, riskTolerance: 45 }),
      })
      const payload = await result.json() as { variants?: AiVariantResponse[]; error?: string }
      if (!result.ok || !payload.variants) throw new Error(payload.error || 'Не удалось создать варианты')
  ...
  const saveVariant = async (variantId: string, approve: boolean) => {
    ...
    try {
      const draft = await createQuickDraft(variant.text, contentFormat as ContentFormat)
    ...
    <AppShell>
      <div className="studio-layout">
        ...
        <div className="studio-config">
          ...
          <Card><SectionTitle icon={<Bot />} title="Контекст" /><label>Основная мысль<textarea rows={9} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Например: объяснить, почему системный контент сильнее случайных вирусных постов" /></label><label>Цель аудитории<input placeholder="Получить ответы и переходы" /></label><Button className="full-button" onClick={() => void generate()} disabled={loading || prompt.trim().length < 5}><Sparkles size={18} /> {loading ? 'Генерируем...' : 'Создать варианты'}</Button></Card>
        </div>
